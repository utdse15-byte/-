...
async function main() {
    ...
    socket.on('message', (raw) => {
      ...
      switch (message.method) {
        ...
        case 'DOM.describeNode':
          // v6.7 起控制器只读文件输入框的 DOM 属性，不在目标页执行脚本。
          result = { node: { nodeId: 0, backendNodeId: message.params?.backendNodeId || 0, nodeName: 'INPUT', attributes: ['type', 'file', 'accept', '.txt,.pdf', 'multiple', ''] } };
          break;
        default:
          result = {};
      }
  ...
  try {
    ...
    // 视口修订仍在协商时，落在旧几何上的手势会按设计被丢弃（REL-001）。
    // 真实用户会重试点击；测试同样以“派发 + 短等待 + 重试”的方式验证通路。
    const dispatchStageTap = (pointerId) => pageCdp.evaluate(`(() => {
      const stage = document.getElementById('stage');
      const rect = stage.getBoundingClientRect();
      const init = { bubbles: true, cancelable: true, pointerId: ${pointerId}, pointerType: 'touch', isPrimary: true, clientX: rect.left + rect.width / 2, clientY: rect.top + rect.height / 2 };
    ...
    for (let attempt = 0; attempt < 4 && !tapDelivered; attempt += 1) {
      await dispatchStageTap(17 + attempt);
      tapDelivered = await waitFor(
        () => observed.filter((item) => item.method === 'Input.dispatchTouchEvent').length >= touchBefore + 2,
        2500, 60, '触摸注入'
      ).then(() => true).catch(() => false);
    }
    assert.ok(tapDelivered, '轻点应在视口稳定后注入 Input.dispatchTouchEvent');
    ...
    const dispatchStageScroll = (pointerId) => pageCdp.evaluate(`(() => {
      const stage = document.getElementById('stage');
      const rect = stage.getBoundingClientRect();
      const base = { bubbles: true, cancelable: true, pointerId: ${pointerId}, pointerType: 'touch', isPrimary: true, clientX: rect.left + rect.width / 2 };
      stage.dispatchEvent(new PointerEvent('pointerdown', { ...base, clientY: rect.top + rect.height * 0.58 }));
      stage.dispatchEvent(new PointerEvent('pointermove', { ...base, clientY: rect.top + rect.height * 0.34 }));
      stage.dispatchEvent(new PointerEvent('pointerup', { ...base, clientY: rect.top + rect.height * 0.34 }));
      return true;
    })()`);
    let scrollDelivered = false;
    for (let attempt = 0; attempt < 4 && !scrollDelivered; attempt += 1) {
      await dispatchStageScroll(31 + attempt);
      scrollDelivered = await waitFor(
        () => observed.slice(scrollObservedAt).some((item) => (item.method === 'Input.dispatchMouseEvent' || item.method === 'Input.emulateTouchFromMouseEvent') && item.params.type === 'mouseWheel'),
        2500, 60, '智能滚动注入'
      ).then(() => true).catch(() => false);
    }
    assert.ok(scrollDelivered, '智能滑动应注入滚轮事件');
    ...
    const chooserEventCount = observed.filter((item) => item.method === 'DOM.describeNode').length;
    edgeSocket.send(JSON.stringify({
      method: 'Page.fileChooserOpened',
      sessionId: 'session-ui',
      params: { backendNodeId: 77, frameId: 'frame-1', mode: 'selectMultiple' }
    ...
    assert.strictEqual(observed.filter((item) => item.method === 'DOM.describeNode').length, chooserEventCount,
