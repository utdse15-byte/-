...
const notes = read('更新说明-v6.7.0.md');
...
assert.strictEqual(packageJson.version, '6.7.0');
assert.strictEqual(lock.version, '6.7.0');
assert.strictEqual(lock.packages[''].version, '6.7.0');
assert.strictEqual(installedLock.version, '6.7.0');
assert.strictEqual(versionText, '6.7.0');
assert.ok(readme.startsWith('# Edge 手机 CDP 控制器 v6.7.0'));
assert.ok(notes.startsWith('# v6.7.0 更新说明'));
assert.ok(report.startsWith('# Edge 手机 CDP 控制器 v6.7.0 发布验证报告'));
assert.ok(server.includes("const VERSION = '6.7.0'"));
