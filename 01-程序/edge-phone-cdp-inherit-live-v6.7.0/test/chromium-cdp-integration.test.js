...
async function main() {
  ...
  try {
    ...
    // Force a wide real CSS/frame viewport without changing the phone-side
    // stage. This proves that coordinates are bound to the exact current frame
    // dimensions instead of a cached 390px viewport from another page/state.
    ...
    await observer.send('Emulation.setDeviceMetricsOverride', {
      width: 980, height: 1750, deviceScaleFactor: 2, mobile: true,
      screenWidth: 980, screenHeight: 1750,
      screenOrientation: { type: 'portraitPrimary', angle: 0 }
    });
    await waitFor(() => observer.evaluate('innerWidth >= 900'), 10000, '宽 CSS 视口');
    reply = await sendRequest('recoverFrame', {}, 20000);
    assert.strictEqual(reply.ok, true);
    const wideCssFrame = await waitFrame(
      (item) => Number(item.metadata.sequence) > previousSequence && Number(item.metadata.viewportRevision) === initialRevision &&
        Number(item.metadata.deviceWidth) >= 900,
      scaledStartedAt,
      20000,
      '宽页面对应的新画面'
    );
    ...
    await waitFor(() => observer.evaluate('innerWidth === 390'), 10000, '恢复设备宽度视口');
    // The observer uses a separate CDP session. Clear its temporary override so
    // the controller session is again the sole owner of the viewport before the
    // fullscreen transition is verified.
    await observer.send('Emulation.clearDeviceMetricsOverride');
    ...
    console.log('chromium-cdp-integration.test.js: OK (真实 Chromium 顶部第一行、不同页面比例、全屏尺寸修订、三点校准标记、命中测试、原生触摸、文件上传、桌面视口与 v6.7 严格人工模式)');
  } catch (error) {
