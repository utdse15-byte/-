...
class CdpController {
  ...
  async handleUiaState(state = {}) {
    ...
    const promise = (async () => {
      ...
      const matched = chooseTargetFromUia(targets, state);
      if (!matched?.target) {
        const now = Date.now();
  ...
  async ensureManualCompatibilityWindow() {
    ...
    this.manualCompatibilityWindowId = windowId;
    if (!this.manualCompatibilitySavedWindows.has(windowId)) {
  ...
  async captureSnapshot(source = 'snapshot', force = false) {
    if (!this.isOpen()) return false;
    const now = Date.now();
    ...
    this.snapshotPromise = (async () => {
      ...
      try {
        if (force && !this.followDesktopTabs) {
        ...
        const baseParams = {
          format: 'jpeg',
          quality: clampInt(this.effectiveStreamSettings().quality, 30, 95, SCREENCAST_QUALITY),
          fromSurface: true,
        ...
        this.publishFrame(Buffer.from(result.data, 'base64'), metadata);
        this.consecutiveSnapshotFailures = 0;
        this.lastSnapshotSuccessAt = Date.now();
        return true;
      } catch (error) {
